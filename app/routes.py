...
# Query expansion (optional but keeps intelligence)
...
async def query_paper(req: QueryRequest):
    ...
    for q in expanded_queries:
        try:
            ...
            continue
        # 👇 FIX: only filter if paper_id is set and not "all"
        filter_dict = {}
        if req.paper_id and req.paper_id != "all":
            filter_dict["source"] = req.paper_id
        try:
